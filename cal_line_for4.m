...
y0_down = E5_down;
y0_up = E5_up-1E-6;
% y0_up = y0_down+0.00001;
% y0_down = E5_down+0.003;
% y0_up = y0_down+0.0001;
    ...
    while test_g ~= 1
        ...
        if Z == 1 && m_1(sz_x,1)<0 &&  m_1(sz_x,1)<-10%折射+斜率是負+(打到外徑)
            if abs(x_1(sz_x,1)^2+y_1(sz_x,1)^2-r_in^2)>abs(x_1(sz_x,1)^2+y_1(sz_x,1)^2-r_out^2) %折射+斜率是負+打到外徑
                ...
                if abs(x_1(sz_x,1)^2+y_1(sz_x,1)^2-r_in^2)<abs(x_1(sz_x,1)^2+y_1(sz_x,1)^2-r_out^2) %折射+斜率是負+打到內徑
                    for x_f = x_1(sz_x,1) : -kankaku_1 : -r_out
                        y_f = x_f*m_1(sz_x,1)+b_1(sz_x,1);
                        hit_choose = hit_f_choose(sz_x);
                        [hit,n_1,n_2] = hit_function(hit_choose,x_f,y_f,n_A,n_B,n_C,r_out,r_in);
                    ...
                    for x_f = -r_in : kankaku_1 : r_in
                        ...
                        if sz_x ~=4
                            ...
                            if hit == 1
                                ...
                                [th_c,test_real,th_t,I_cal,Z,th_mt,R_s,R_p,R_1,T_1,m_1,b_1] = function_zenhansha(n_1,n_2,m_n,m_1,sz_x,th_i,I_cal,th_mi,th_mn,x_1,y_1,b_1);
                                if Z == 0
                                    x_1(sz_x+1,1) = x_f-kankaku_1;
                                    y_1(sz_x+1,1) = (x_f-kankaku_1)*m_1(sz_x,1)+b_1(sz_x,1);                                    
                                end
                                break
                            else
                                clear hit
...
[file_title_d,file_title_D] = title_fun(d,D);
    ...
    save_filename = ['E6_data_line4_' file_title_D '_' file_title_d '.mat'];
